...
class _DetailBody extends ConsumerWidget {
  ...
  Future<void> _handleFavTap(BuildContext context, WidgetRef ref, bool isAuth,
  ...
  _Rating? _resolveRating(Vehicle v, BuildContext context) {
    ...
    switch (v.priceRating) {
      case 'very_good':
        ...
        bg = const Color(0x1F16A34A);
        break;
      case 'good':
        color = const Color(0xFF4D7C0F);
        bg = const Color(0x1F84CC16);
        break;
      case 'fair':
        color = const Color(0xFFB45309);
        bg = const Color(0x1FEAB308);
        break;
      case 'high':
        color = const Color(0xFFC2410C);
        bg = const Color(0x1FF97316);
        break;
      case 'very_high':
...
class _SpecsGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final specs = [
      _Spec(
          Icons.calendar_today_outlined, '${vehicle.year}', l.vehicleYearLabel),
      _Spec(Icons.speed_outlined,
          '${NumberFormatters.formatKm(vehicle.mileageKm)} km', 'km'),
      _Spec(Icons.local_gas_station_outlined, _fuelLabel(vehicle.fuel, l),
          l.vehicleFuelLabel),
      _Spec(Icons.settings_outlined, _transLabel(vehicle.transmission, l),
          l.vehicleTransmission),
      _Spec(
...
class _CtaDock extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final l = AppLocalizations.of(context)!;
    final isAr = Localizations.localeOf(context).languageCode == 'ar';
    final phone = vehicle.agency?.phoneWhatsapp ??
        vehicle.agency?.phone ??
        vehicle.user?.phoneWhatsapp ??
        vehicle.user?.phoneCall;
        ...
        children: [
          ...
          Expanded(
            child: GestureDetector(
              onTap:
                  hasPhone ? () => _openWhatsApp(phone, vehicle, isAr) : null,
              child: Container(
                height: 50,
                decoration: BoxDecoration(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (agency.vehiclesCount != null)
                            Text(
                                '${agency.vehiclesCount} ${l.agencyActiveListings}',
                                style: const TextStyle(
                                    fontSize: 12,
                                    color: AppColors.textSecondary)),
                          if (agency.city != null)
...
class _OtherAgencyVehicles extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final async = ref.watch(
        agencyVehiclesProvider((agencyId: agencyId, excludeId: excludeId)));
                  ...
                  children: [
                    Text(l.otherVehicles,
                        style: const TextStyle(
                            ...
                            letterSpacing: -0.3)),
                    Text(
                        '$agencyName · ${vehicles.length} ${l.agencyActiveListings}',
                        style: const TextStyle(
                            fontSize: 12, color: AppColors.textSecondary)),
                  ],
                ),
...
final _reviewSummaryProvider = FutureProvider.family<Map<String, dynamic>,
    ({String sellerId, String sellerType})>((ref, args) async {
  final dio = ref.read(apiClientProvider);
  final path = args.sellerType == 'user'
      ? '/sellers/users/${args.sellerId}/rating-summary'
      : '/sellers/agencies/${args.sellerId}/rating-summary';
  final r = await dio.get(path);
  return r.data as Map<String, dynamic>;
});

final _reviewListProvider = FutureProvider.family<List<dynamic>,
    ({String sellerId, String sellerType})>((ref, args) async {
  final dio = ref.read(apiClientProvider);
  final path = args.sellerType == 'user'
      ? '/sellers/users/${args.sellerId}/reviews'
      : '/sellers/agencies/${args.sellerId}/reviews';
  final r = await dio.get(path);
  final data = r.data;
...
final _canReviewProvider =
    FutureProvider.family<Map<String, dynamic>, String>((ref, vehicleId) async {
  try {
    final dio = ref.read(apiClientProvider);
...
class _ReviewsSectionState extends ConsumerState<_ReviewsSection> {
  ...
  Future<void> _submit(AppLocalizations l) async {
    ...
    try {
      ...
      setState(() {
        _success = l.reviewsSuccess;
        _showForm = false;
      });
      ref.invalidate(_reviewSummaryProvider(
          (sellerId: widget.sellerId, sellerType: widget.sellerType)));
      ref.invalidate(_reviewListProvider(
          (sellerId: widget.sellerId, sellerType: widget.sellerType)));
  ...
  Widget build(BuildContext context) {
    ...
    final isAr = Localizations.localeOf(context).languageCode == 'ar';
    final isAuth = ref.watch(isAuthenticatedProvider);
    final summary = ref.watch(_reviewSummaryProvider(
        (sellerId: widget.sellerId, sellerType: widget.sellerType)));
    final reviews = ref.watch(_reviewListProvider(
        (sellerId: widget.sellerId, sellerType: widget.sellerType)));
                ...
                child: Row(children: [
                  ...
                  Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 2),
                        Text(
                            '${l.reviewsBasedOn} $count ${l.reviewsCount(count)}',
                            style: const TextStyle(
                                fontSize: 12, color: AppColors.textSecondary)),
                      ]),
                ]),
